    ...
    public partial class frmSearch : Form
    {
        ...
        private void btnRun_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //This is to display empty DatagridView if search is not valid instead of displaying error
                ...
                {
                    //This is to allow Multiply input options for 'Available' filter such as True, Yes and 1 (also allowing Insensitive Case)
                    if (Field == "Available" && txtValue.Text.Equals("Yes", StringComparison.CurrentCultureIgnoreCase) || Field == "Available" && txtValue.Text.Equals("True", StringComparison.CurrentCultureIgnoreCase) || Field == "Available" && txtValue.Text == "1")
                    {
                        dv.RowFilter = string.Format(Field + Operator + "'{0}'", "True");
                        dgvSearch.DataSource = dv.ToTable();
                    }
                    //This is to allow Multiply input options for 'Available' filter such as False, No and 0 (also allowing Insensitive Case)
                    else if (Field == "Available" && txtValue.Text.Equals("No", StringComparison.CurrentCultureIgnoreCase) || Field == "Available" && txtValue.Text.Equals("False", StringComparison.CurrentCultureIgnoreCase) || Field == "Available" && txtValue.Text == "0")
                    {
